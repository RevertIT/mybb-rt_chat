...
class Read
{
    ...
    public function getMessages(): mixed
    {
        ...
        if (isset($mybb->settings['rt_chat_mycode_enabled']) && (int) $mybb->settings['rt_chat_mycode_enabled'] === 1)
        {
            $parser_options['allow_mycode'] = 1;
        }
        if (isset($mybb->settings['rt_chat_smilies_enabled']) && (int) $mybb->settings['rt_chat_smilies_enabled'] === 1)
        {
        ...
        if (empty($this->messages['messages']))
        {
            ...
            $query = $db->write_query("
                ...
                ORDER BY c.id DESC
                LIMIT {$mybb->settings['rt_chat_total_messages']}
            ");
            ...
            foreach ($query as $key => $row)
            {
                ...
                $row['avatar'] = !empty($row['avatar']) ? htmlspecialchars_uni($row['avatar']) : "{$mybb->settings['bburl']}/images/default_avatar.png";
                $row['username'] = isset($row['uid'], $row['username'], $row['usergroup'], $row['displaygroup']) ? build_profile_link(format_name($row['username'], $row['usergroup'], $row['displaygroup']), $row['uid']) : null;
            ...
            if (empty($messages))
            {
                ...
                $final_data['error'] = $lang->rt_chat_no_messages_found;
            }
